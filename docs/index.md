...
features:
  - title: For users
    details: Connect the LLM provider you actually want — Anthropic, OpenAI, or your own self-hosted model — once, from the extension's own settings. Approve which sites can use it, one at a time. Your API key stays in the extension; a connected site never sees it.
  - title: For developers
    details: window.aquilifer is all you need — no backend proxy to stand up, deploy, and pay for just to keep an API key off the client. One check for whether it's installed, one call for a completion, with a fallback path for when it isn't.
  - title: For companies
...
That also means the user — not the site — decides which provider answers, keeps their own audit log of what was asked, and sets their own rate limits. A site that wants this as a genuine fallback (its own integration first, Aquilifer only if that's unavailable) can do that too, without ever risking the user's spend limits or credentials.
